...
BASE_URL="https://beta.unity3d.com/download/0e6151d5f054"
VERSION="2019.1.0a14"

# https://beta.unity3d.com/download/0e6151d5f054/MacEditorInstaller/Unity.pkg


UNITY_OSX_PACKAGE="MacEditorInstaller/Unity.pkg"
...
download() {
	...
	if [ ! -e $UNITY_DOWNLOAD_CACHE/`basename "$FILE"` ] ; then
		...
		sudo curl -o $UNITY_DOWNLOAD_CACHE/`basename "$FILE"` "$URL"
	else
